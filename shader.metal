...
vertex SolidVertexOutput SDL_Solid_vertex(
    const device float2 *position [[buffer(0)]],
    constant float4x4 &projection [[buffer(2)]],
    constant float4x4 &transform [[buffer(3)]], uint vid [[vertex_id]]) {
  SolidVertexOutput v;
...
vertex CopyVertexOutput SDL_Copy_vertex(
    const device float2 *position [[buffer(0)]],
...
fragment float4 SDL_YUV_fragment(CopyVertexOutput vert [[stage_in]],
  ...
  return col * float4(dot(yuv, decode.Rcoeff), dot(yuv, decode.Gcoeff),
